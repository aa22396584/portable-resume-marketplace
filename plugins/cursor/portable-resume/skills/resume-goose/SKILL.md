---
name: resume-goose
description: "Resume or continue the last goose session — pick up previous work, import inert offline handoff context into a fresh session (never live process restore)."
---

# resume-goose

Import inert local **goose** session context into a **fresh** session.
...
`SKILL.md` (not another copy of `resume-goose` found by name under cwd
...
3. Never search bare `resume-goose` under `$PWD` or foreign roots.

Do **not** call the goose CLI. Prefer a host tool API that passes
...
The wrapper hard-binds `source=goose` and loads the installer-owned
...
1. Write a private temp file (restrict permissions if your tools allow; the
   ...
   - `source`: must equal this Skill's bound source (`goose`)
