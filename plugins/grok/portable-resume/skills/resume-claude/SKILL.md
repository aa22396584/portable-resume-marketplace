---
name: resume-claude
description: "Resume or continue the last Claude Code session — pick up previous work, import inert offline handoff context into a fresh session (never live process restore)."
---
...
The wrapper hard-binds `source=claude` and loads the installer-owned
...
1. Write a private temp file (restrict permissions if your tools allow; the
   ...
   - `source`: must equal this Skill's bound source (`claude`)
