---
name: resume-cline
description: "Resume or continue the last Cline session — pick up previous work, import inert offline handoff context into a fresh session (never live process restore)."
---

# resume-cline

Import inert local **Cline** session context into a **fresh** session.
...
`SKILL.md` (not another copy of `resume-cline` found by name under cwd
...
3. Never search bare `resume-cline` under `$PWD` or foreign roots.

Do **not** call the Cline CLI. Prefer a host tool API that passes
...
The wrapper hard-binds `source=cline` and loads the installer-owned
...
1. Write a private temp file (restrict permissions if your tools allow; the
   ...
   - `source`: must equal this Skill's bound source (`cline`)
