---
name: resume-crush
description: "Resume or continue the last Crush session — pick up previous work, import inert offline handoff context into a fresh session (never live process restore)."
---

# resume-crush

Import inert local **Crush** session context into a **fresh** session.
...
`SKILL.md` (not another copy of `resume-crush` found by name under cwd
...
3. Never search bare `resume-crush` under `$PWD` or foreign roots.

Do **not** call the Crush CLI. Prefer a host tool API that passes
...
The wrapper hard-binds `source=crush` and loads the installer-owned
...
1. Write a private temp file (restrict permissions if your tools allow; the
   ...
   - `source`: must equal this Skill's bound source (`crush`)
