...
description: "Resume or continue the last OpenClaw session — pick up previous work, import inert offline handoff context into a fresh session (never live process restore)."
---

# resume-openclaw

Import inert local **OpenClaw** session context into a **fresh** session.
...
`SKILL.md` (not another copy of `resume-openclaw` found by name under cwd
...
3. Never search bare `resume-openclaw` under `$PWD` or foreign roots.

Do **not** call the OpenClaw CLI. Prefer a host tool API that passes
...
The wrapper hard-binds `source=openclaw` and loads the installer-owned
...
1. Write a private temp file (restrict permissions if your tools allow; the
   ...
   - `source`: must equal this Skill's bound source (`openclaw`)
