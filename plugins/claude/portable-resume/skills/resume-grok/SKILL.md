---
name: resume-grok
description: "Resume or continue the last Grok Build session — pick up previous work, import inert offline handoff context into a fresh session (never live process restore)."
---
...
`SKILL.md` (not another copy of `resume-grok` found by name under cwd
...
3. Never search bare `resume-grok` under `$PWD` or foreign roots.

Do **not** call the Grok Build CLI. Prefer a host tool API that passes
...
The wrapper hard-binds `source=grok` and loads the installer-owned
...
1. Write a private temp file (restrict permissions if your tools allow; the
   ...
   - `source`: must equal this Skill's bound source (`grok`)
