...
description: "Resume or continue the last OpenCode session — pick up previous work, import inert offline handoff context into a fresh session (never live process restore)."
---
...
3. Never search bare `resume-opencode` under `$PWD` or foreign roots.
...
1. Write a private temp file (restrict permissions if your tools allow; the
   ...
   - `source`: must equal this Skill's bound source (`opencode`)
